...
const Footer = () => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus({ loading: false, success: false, error: data.message || 'Subscription failed' });
        return;
      }
      ...
      setStatus({ loading: false, success: false, error: 'Something went wrong.' });
    }
  };
            ...
            <div className="pt-2">
              <div className="flex items-center border border-[#8C7A5C] bg-[#4D3E24] rounded-sm overflow-hidden focus-within:border-white transition-colors">
                ...
                <button
                  ...
                  className="bg-[#6E5B3A] border-l border-[#8C7A5C] px-3 py-2 flex items-center justify-center hover:bg-[#856F48] transition-colors disabled:opacity-50"
                >
              ...
              {status.success && (
                <p className="text-xs text-green-300 mt-2">Subscribed! Check your inbox.</p>
              )}
              {status.error && (
                <p className="text-xs text-red-300 mt-2">{status.error}</p>
              )}
            </div>
          </div>
