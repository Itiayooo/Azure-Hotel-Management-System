...
const Navbar = ({ className }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 flex justify-between items-center py-5 px-6 md:px-12 text-white font-['Mona_Sans'] z-[99999] w-full">
      {/* Brand Logo */}
      ...
      <div className="hidden lg:flex justify-between items-center w-[60%] lg:w-[50%]">
        <ul className="flex list-none gap-[30px] m-0 p-0">
          {navLinks.map((link) => (
            <li key={link.name}>
              <Link
                to={link.path}
                className={`text-white no-underline text-[16px] relative py-1 transition-colors duration-200 hover:text-[#b68b47] ${location.pathname === link.path
                    ? 'font-medium text-[#b68b47]'
                    : 'opacity-90'
                  }`}
              >
                {link.name}
        ...
        <div className="flex items-center justify-center gap-[15px]">
          {user ? (
            <div className="flex items-center gap-[10px] py-[6px] px-[14px] rounded-full bg-[#5E5C5C75] backdrop-blur-[63px]">
      ...
      >
        <span
          className={`w-5 h-[1.5px] bg-white rounded-full transition-all duration-300 ease-in-out ${isOpen ? 'rotate-45 translate-y-[6.5px]' : ''
            }`}
        ></span>

        <span
          className={`w-5 h-[1.5px] bg-white rounded-full transition-all duration-300 ease-in-out ${isOpen ? 'opacity-0' : ''
            }`}
        ></span>
